...
class _DashboardState extends State<Dashboard> with WidgetsBindingObserver {
  ...
  ///
  /// Замерено: Apple хранит 8 отчётов на ключ, всего 40 на метку. Сколько
  /// времени эти 40 покрывают, зависит от того, как быстро они появляются:
  ///
  ///   метка в движении, ~40 отчётов в час  ->  буфер на 62 минуты
  ///   метка стоит среди телефонов          ->  буфер на 14 минут
  ///
  /// Обе цифры получены прямым замером: свежий клиент с пустой историей
  /// выгружали сразу после добавления метки, и он мог показать только то,
  /// что Apple держит на этот момент.
  ///
  /// Четырнадцать минут выбраны сознательно как компромисс: для основной
  /// задачи — понимать, где объект, — этого достаточно, а частые запросы
  /// нагружают Apple и делают трафик менее похожим на человеческий. В
  /// плотной обстановке часть отчётов при таком интервале будет теряться;
  /// закрывается это не клиентским таймером, а серверным сборщиком.
  static const int refreshIntervalMinutes = 14;
